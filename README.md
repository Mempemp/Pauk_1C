...
JAR и секреты в репозиторий Pauk **не** кладутся: серверы ставятся локально, в настройках MCP IDE (например `.cursor/mcp.json`). Оснастка **работает без MCP**; при подключении серверов из этого раздела обычно выше качество справки по тем областям, для которых сервер предназначен (например платформенный API). Ниже — отдельный пункт на каждый такой сервер (список будет пополняться).
...
3. В конфигурации MCP: `java` … `-jar` **абсолютный путь к jar** `--platform-path` **каталог установки платформы 1С** (на Windows часто добавляют `-Dfile.encoding=UTF-8` — см. README репозитория).

Маршрут для агента после копирования пакета: **`pauk/routing/MCP-CATALOG.md`**, **`pauk/mcp/route-bsl-platform-context.md`**.
