...
**Готовый сценарий на большую задачу** (цепочка с планом и фазами): в поставке это файл **`.cursor/commands/flow-full-pipeline.md`** в **корне** проекта рядом с `pauk/`. В средах с **командами** по папке `.cursor/commands` его можно вызвать из палитры; **в других** — открыть тот же markdown в репозитории и ориентироваться на шаги и ссылки (пресет `full-pipeline` в `pauk/subagents/WORKFLOW.md`, шаблон плана `pauk/routing/ARTIFACT-ROUTER.md`).
...
- Для **заранее оговорённого полного контура** (анализ → архитектура/blueprint → реализация → ревью → приёмка): взять готовое описание — **`flow-full-pipeline.md`** (см. путь в блоке «Схема» выше). Смысл: одна **та же** последовательность шагов в любом инструменте — через **команду** там, где она есть, иначе — открыть markdown и идти по ссылкам. Детали этапов и вариации — **`pauk/subagents/WORKFLOW.md`** (раздел про предопределённые сценарии и пресет **full-pipeline**).
