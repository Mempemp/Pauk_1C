...
```mermaid
flowchart TD
  Start([Вход задачи]) --> Gate{Явно крупная / full?}
  Gate -->|нет, по умолчанию| Impl[Implement]
...
- **До диаграммы:** по умолчанию задача **не** крупная (`scale: trivial`); отдельный вопрос модели про «мелкая / нетривиально» не задаётся. **`scale: full`** — только если человек **явно** обозначил крупный объём (см. [ROUTING-ENTRY.md](ROUTING-ENTRY.md), [PROTOCOL.md](../pauk-product/pauk/routing/PROTOCOL.md)). Для `full` первое сообщение — короткая подсказка без полного артефакта Task Analyst; для `trivial` — сразу работа в том же сообщении. YAML `route` в чат — по запросу пользователя (см. PROTOCOL).
- **Обычная (trivial) задача** — эвристика в оркестрации: локальное изменение без отдельного тяжёлого аналитического контура — цепочка укорачивается до **Implement → Verify** (при необходимости лёгкий Explore без отдельного вызова).
- **Крупная (full) задача** — полная цепочка с **Task Analyst** в начале (после отдельного лёгкого первого сообщения, см. ROUTING-ENTRY).
